...
Python CLI tool (v0.7.0) that evaluates agent skills (SKILL.md files) via static analysis, LLM-as-judge quality review, trigger testing, and LLM-based test generation. Produces a 0-100 static score across 5 dimensions (Structure, Naming, Description, Content, Security) plus a 0-100 LLM judge score across 9 criteria / 2 axes (Activation Quality + Instruction Quality). For live check counts run `sklab list-checks`.
...
| [CONTRIBUTING.md](CONTRIBUTING.md) | Dev setup, "Adding a New Check" walkthrough |

For changes that touch multiple modules, read `docs/ARCHITECTURE.md` first. After code changes: update `ARCHITECTURE.md` (modules/CLI) and the relevant `docs/versions/vX.X.X.md`.
...
- `/verify` — runs the full verification suite (pytest, mypy, ruff check, ruff format)
- `/update-counts` — resyncs check counts across docs and tests after adding/removing checks
...
- **Two check systems**: behavioral (`@register_check` classes in `structure.py`, `naming.py`, `content.py`, `security.py`) and schema-based (`FieldRule` in `schema.py` — append to add a check, no class needed). Per-file `@register_check` counts: structure:9, schema:9 (one factory registration per `FieldRule`), content:13, security:5, naming:1. Treat per-file counts as the source of truth — dimension counts are derived from each check's `dimension =` field. Verify with `grep @register_check src/skill_lab/checks/static/`.
...
- **Check count maintenance**: When adding/removing checks, update the count language at the top of this file and run `/update-counts` to sync docs and tests.
...
- **Required SDKs**: `anthropic`, `openai`, and `google-generativeai` are all required dependencies. API key env vars: `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`.

### Other Required Dependencies

- **`docker>=7.0.0`** is a hard dependency (not optional) because `providers/docker_provider.py` is imported alongside `LocalProvider`. Even users running only `--provider local` will install it.
...
- **`.env` support**: `python-dotenv` loads `.env` from CWD at CLI startup inside `skill_lab.cli:main` (the pyproject `[project.scripts]` entry point). Real env vars override `.env` values (`override=False`). Entry point is `cli:main` rather than `cli:app` so dotenv loads before Typer dispatch.
...
- **Attribution**: NEVER include Co-Authored-By lines, "Generated with Claude Code", or any AI co-authorship attribution in commit messages, PR descriptions, PR reviews, or any other output.
