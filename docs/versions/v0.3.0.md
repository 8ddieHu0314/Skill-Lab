# v0.3.0: LLM-as-Judge & Auto-Fix

**Status:** Planned

## Overview

LLM-as-judge failure analysis for trigger tests with actionable fix suggestions and interactive auto-fix capability. The previous rule-based `FailureAnalyzer` (bag-of-words keyword matching) was removed due to brittleness — this version replaces it with LLM-powered semantic analysis.

## Features

### Failure Analysis (LLM-as-Judge)
- LLM-powered analysis explaining WHY tests failed
- Semantic understanding of prompt intent vs skill description
- Actionable fix suggestions with confidence levels
- Replaces the removed rule-based `FailureAnalyzer`
...
```bash
# Trigger testing with failure analysis
sklab trigger ./my-skill                    # All tests + analysis + suggestions
...
Shows all tests, LLM-powered analysis for failures, and suggestions:
...
negative-5: FAIL (expected no_trigger, got trigger)
  ...
  Analysis: Triggered because prompt asks to execute a commit,
            but the skill is for drafting commit messages.
            The LLM couldn't distinguish execution from drafting.
...
The previous rule-based approach (FP-1 through FP-4, FN-1 through FN-4) used bag-of-words keyword matching with hardcoded rules. This was removed because:
- Keyword matching can't explain semantic LLM decisions
- Verb classification was brittle ("create a commit message" has execution verb but user wants drafting)
- Pattern matching was too narrow for real-world prompts
- Confidence scores were static with no calibration

The v0.3.0 approach uses LLM-as-judge to analyze the prompt, skill description, and test result to provide semantic root cause analysis.
...
src/skill_lab/
├── triggers/
│   ├── failure_analyzer.py     # NEW: LLM-as-judge failure analysis
...
- [ ] `FailureAnalyzer` class with LLM-as-judge analysis
- [ ] LLM provider abstraction for failure analysis
- [ ] Enhanced `_print_trigger_report()` with analysis display
- [ ] Unit tests for failure analyzer (with mocked LLM responses)
