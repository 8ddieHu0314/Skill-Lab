...
- `description.py` deleted — `description.includes-triggers` removed (deferred to LLM-as-judge)
...
**Check count:** 18 (v0.1.0) → 19 (added `structure.standard-frontmatter-fields` + `frontmatter.license-format`, removed `description.includes-triggers`)

### Removed: `description.includes-triggers`

Static regex matching for trigger keywords (`when`, `if`, `trigger`, etc.) produces false positives on descriptions that clearly convey intent without using those exact words. For example, "Research a topic from the last 30 days" obviously describes when to use the skill but fails the keyword check.

This check is deferred to a future release as an LLM-as-judge evaluation, which can assess semantic clarity rather than keyword presence.
...
- **Description trigger clarity**: Does the description convey when to use this skill?
