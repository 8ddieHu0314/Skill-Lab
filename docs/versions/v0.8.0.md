...
The following script-related checks were deferred from the script checks batch and may be added in this or a future version:

- [ ] **Version pinning check** — Warn when scripts use unpinned dependencies (e.g., `pip install requests` without version)
- [ ] **`--help` support check** — Verify that scripts accept a `--help` flag for discoverability
